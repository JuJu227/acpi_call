...
*/
static void do_acpi_call(const char *method, int argc,
...
@param args
*/
static char *parse_acpi_args(char *input, int *nargs,
  ...
  *args = (union acpi_object *)kmalloc(
      MAX_ACPI_ARGS * sizeof(union acpi_object), GFP_KERNEL);
      ...
      if (*s == '"') {
        ...
        while (*s && *s++ != '}') {
          if (buf >= temporary_buffer + sizeof(temporary_buffer)) {
            printk(KERN_ERR "acpi_call: buffer arg%d is truncated because "
                            "the buffer is full\n",
                   *nargs);
...
/** procfs 'call' read callback. Called when reading the content of
/proc/acpi/call. Returns the last call status:
...
#else
static int acpi_proc_read(char *page, char **start, off_t off, int count,
                          int *eof, void *data) {
  int len = 0;
...
static int __init init_acpi_call(void) {
  ...
  struct proc_dir_entry *acpi_entry =
      proc_create("call", 0660, acpi_root_dir, &proc_acpi_operations);
#else
  struct proc_dir_entry *acpi_entry =
      create_proc_entry("call", 0660, acpi_root_dir);
#endif
